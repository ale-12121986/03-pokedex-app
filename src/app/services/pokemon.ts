...
export class PokemonService {
  ...
  private readonly URL_BASE:string = `https://pokeapi.co/api/v2/pokemon/`; 
 ...
 getPokemon(){
  ...
  if(this.nextUrl) {
    ...
    return CapacitorHttp.get(option).then(async (response:HttpResponse) => {
      ...
      if(response.data){
        ...
        for (const result of results) {
          ...
          promises.push(CapacitorHttp.get(optionPokemon));
        }
  ...
  private processPokemon(pokemonData: any){
    ...
    if(hiddenAbilities) {
      pokemon.hiddenAbilities = hiddenAbilities.ability.name;
    }
    return pokemon;
  }
